...
interface Agent { id: string; name: string; agent_id: string; api_key: string; emoji: string; color: string; image_url: string | null }
...
function ExportModal({ convs, onClose }: { convs: Conversation[]; onClose: () => void }) {
  ...
  const [fields, setFields]   = useState({ name:true, date:true, summary:true, kb:true, nonkb:true, chat:false })
  ...
  function doDownload(rows: Conversation[]) {
    ...
    const header = [...(fields.name?['User Name']:[]),...(fields.date?['Date']:[]),...(fields.summary?['Transcript Summary']:[]),...(fields.kb?['KB Questions']:[]),...(fields.nonkb?['Non-KB Questions']:[])]
    if (header.length) {
    ...
    if (fields.chat) {
      const chatRows: any[] = []
      rows.forEach(r=>{ const un=extractUserName(r)||'Unknown',sd=r.start_time_unix?new Date(r.start_time_unix*1000).toLocaleString('en-IN'):'',tr=rowToTranscript(r)
        ...
        else tr.forEach((t,i)=>{ const sp=t.role==='user'?un:'Agent',ts=t.time_in_call_secs!=null?`${String(Math.floor(t.time_in_call_secs/60)).padStart(2,'0')}:${String(t.time_in_call_secs%60).padStart(2,'0')}`:''
        ...
        <div style={{ animation:'fadeUp .3s ease' }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:'14px',marginBottom:'22px' }}>
            ...
            <div style={{ ...S.statCard, animationDelay:'250ms' }}>
              <div style={{ fontSize:'10px',fontWeight:'800',color:'rgba(255,255,255,0.35)',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:'8px' }}>Subscription</div>
              ...
              <div style={S.badge(subBadgeStyle.bg,subBadgeStyle.color,subBadgeStyle.border)}>
                {subStatus.type==='loading'?'Checking…':subStatus.type==='active'?'Active':subStatus.type==='trial'?'Trial':'Inactive'}
              </div>
            </div>
          </div>
